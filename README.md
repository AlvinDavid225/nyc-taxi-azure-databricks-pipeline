...
Production-grade batch data pipeline processing **38.3 million NYC Yellow Taxi trip records** using the **Medallion Architecture** on Azure Databricks. Built to demonstrate real-world data engineering challenges — schema mismatch resolution, data skew handling, broadcast joins, and Delta Lake optimizations.
...
<table>
<tr>
<td><img src="screenshots/bronze_output.png" alt="Bronze Layer Output"/></td>
<td><img src="screenshots/silver_output.png" alt="Silver Layer Output"/></td>
</tr>
<tr>
...
| Cluster | Single Node · Standard_D4ds_v4 · 13.3 LTS |
...
| Metadata added | ingestion_timestamp · source_file · pipeline_name |
...
df_cleaned = (df_bronze
    .filter(col("fare_amount") > 0)                          # no negative fares
    .filter(col("trip_distance") > 0)                        # must have moved
    .filter(col("passenger_count").between(1, 6))            # valid capacity
    .filter(col("PULocationID").isNotNull())                 # valid locations
...
**Broadcast Join** — zone lookup table has only 265 rows. Sending it to all executors eliminates the need to shuffle 35M rows across the network.
...
**Data Skew:** Manhattan has 88% of all trips. Without handling, one Spark worker processes 31M rows while others process 180K — a 170x imbalance.

```python
# Salting distributes Manhattan across 4 partitions
SALT_BUCKETS = 4
...
**Z-ORDER** clusters related data in the same files, reducing files scanned per query:
```sql
OPTIMIZE delta.`/delta/gold_revenue_by_zone`
ZORDER BY (pickup_borough, pickup_zone_name)
```

**3 KPI tables produced:**
...
| DBFS mounts disabled | Unity Catalog workspace restriction | Direct `abfss://` paths with OAuth authentication |
...
az ad app create --display-name "databricks-sp"
...
az keyvault secret set --vault-name <vault> --name sp-client-id     --value <value>
az keyvault secret set --vault-name <vault> --name sp-tenant-id     --value <value>
az keyvault secret set --vault-name <vault> --name sp-client-secret --value <value>
```
